...
const TopNav = () => {
    const [sideNav, setSetsideNav] = useState(false)
    return (
    <div className="max-w-[1520px] mx-auto flex justify-between items-center p-4">
        <div onClick={() => setSetsideNav(!sideNav)} className="flex items-center">
        ...
        {
            sideNav ? (<div className="bg-black/60 fixed top-0 left-0 w-full h-screen z-10" onClick={() => setSetsideNav(!sideNav)}></div>) : ("")
        }

        <div className= {
            sideNav ? "fixed top-0 left-0 w-[300px] h-screen bg-white z-10 duration-300"
            : "fixed top-0 left-[-100%] w-[300px] h-screen bg-white z-10 duration-300"    
        }>
            <AiOutlineClose onClick={() => setSetsideNav(!sideNav)} size={25}
